...
abstract class UserDao extends TransactionalIntIdCRUD[User] with IntIdAll[User] {
	...
	protected val entity = UserDao.UserEntity // entity field is needed by the traits
...
object UserDao {
	 ...
	 */
	object UserEntity extends Entity[IntId, User](classOf[User]) {
		val id = intAutoGeneratedPK("id", _.id) // the id is auto-generated and maps to column "id" and field _.id
		val name = string("name", _.name)
		val email = string("email", _.email)
		val password = string("password", _.password)

		def constructor(implicit m: ValuesMap) = new User(name, email, password) with IntId with Persisted {
