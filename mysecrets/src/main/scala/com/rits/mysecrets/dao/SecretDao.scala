...
object SecretDao {
	...
	object SecretEntity extends Entity[IntId, Secret](classOf[Secret]) {
		...
		val user = manyToOne(classOf[User], _.user) // many secrets belong to 1 user 
		var sharedWith = manyToMany(classOf[User], _.sharedWith) // many secrets can be shared amongst many users
		val reminders = oneToMany(classOf[Reminder], _.reminders)
