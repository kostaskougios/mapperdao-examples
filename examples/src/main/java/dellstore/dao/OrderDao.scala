...
object OrderDao {
	object OrderEntity extends Entity[IntId, Order]("orders", classOf[Order]) {
		...
		val customer = manyToOne("customerid", CustomerDao.CustomerEntity, _.customer)
		...
		val orderLines = oneToMany(OrderLineEntity, "orderid", _.orderLines)
	...
	object OrderLineEntity extends SimpleEntity[OrderLine]("orderlines", classOf[OrderLine]) {
		...
		val order = manyToOne("orderid", OrderEntity, _.order)
		val product = oneToOne(ProductDao.ProductEntity, "prod_id", _.product)
