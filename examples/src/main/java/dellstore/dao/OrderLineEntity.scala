 ...
 * )
 */
object OrderLineEntity extends Entity[CustomIntId, OrderLine]("orderlines") {
	...
	def constructor(implicit m) = new OrderLine(orderlineid, order, product, quantity, orderdate) with CustomIntId
}
