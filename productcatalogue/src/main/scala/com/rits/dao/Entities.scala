...
object Entities {
	object ProductEntity extends Entity[IntId, Product](classOf[Product]) {
		...
		val description = string("description", _.title)
	...
	object PriceEntity extends Entity[IntId, Price](classOf[Price]) {
		...
		def constructor(implicit m: ValuesMap) = new Price(currency, unitPrice, salePrice) with IntId with Persisted {
			val id: Int = PriceEntity.id
		}
	}
}